...
public final class CCNChatNet {
	 ...
	 */
	public synchronized void sendMessage(byte[] message) throws ContentEncodingException, IOException {
	...
	public void shutdown() throws IOException {
		...
		if (null != _readString) {
			...
			showMessage(SYSTEM, now(), ("Shutting down " + _namespace + "...").getBytes());
		}
	}
	...
	public void listen() throws ConfigurationException, IOException, MalformedContentNameStringException {
		...
		_readNameString = new CCNByteObject(_friendlyNameNamespace, (byte[])null, SaveType.RAW, tempReadHandle);
		...
		_writeNameString = new CCNByteObject(_friendlyNameNamespace, publishedNameStr.getBytes(), SaveType.RAW, tempWriteHandle);
						...
						if (userFriendlyName.equals("")) {
							...
							if (_readNameString.available()) {
								...
								if (! _readString.getContentPublisher().equals(_readNameString.getContentPublisher())) {
									showMessage(_readString.getContentPublisher(), _readString.getPublisherKeyLocator(), thisUpdate, _readString.byteArray());						 
								} else { 											 
									addNameToHash(_readNameString.getContentPublisher(), _readNameString.string());
									showMessage(_readNameString.string(), thisUpdate, _readString.byteArray());
								}
							} else {
								showMessage(_readString.getContentPublisher(), _readString.getPublisherKeyLocator(), thisUpdate, _readString.byteArray());
							}
	...
	private CCNByteObject _readString;
	private CCNByteObject _writeString;
	
	// We use these for storing the friendly names of users. 
	private CCNByteObject _readNameString;
	private CCNByteObject _writeNameString;
	...
	private void showMessage(PublisherPublicKeyDigest publisher, KeyLocator keyLocator, Timestamp time, byte[] message) {
